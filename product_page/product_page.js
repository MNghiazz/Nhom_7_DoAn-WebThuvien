...
function changeTab(tabIndex) {
  ...
  document
    .querySelectorAll(".nav-wrap ul li")
    [tabIndex - 1].classList.add("act");
...
const list = document.querySelectorAll(
  ".catalog-content-wrap .volume-wrap .volume .chapters > li"
);
...
fetchData(url.productsId(productId), null, async function (bookDetail) {
  ...
  bookInf
    .querySelector(".author-categories .author")
  ...
  bookWrap.innerHTML = `
    ...
    <script>
        ...
        // Update the data-href attribute of the fb-comments div
        const fbCommentsDiv = document.querySelector('.fb-comments');
        fbCommentsDiv.setAttribute('data-href', currentUrl);

        // Reload Facebook Comments plugin to reflect the new data-href
  ...
  readbtn.addEventListener("click", function () {
    if (pdf) {
      ...
      var title = "Oh no :(";
      var body =
        "Nội dung của sách chưa được cập nhật. Các bạn thông cảm hén <3";
      window.alert(title + "\n" + body);
    }
  });
});
